...
execute as @e[type=marker,tag=gm.gui.cm] at @s unless entity @e[type=chest_minecart,sort=nearest,limit=1,tag=gm.gui.ui,distance=..1] run kill @s
...
clear @a *[minecraft:custom_data~{gm.Button:1}]
...
#> teleport marker
execute as @e[type=item_display,tag=gm.vehicle] if score @s gm.Linking = @p[sort=furthest] gm.Linking as @p[limit=1] at @s anchored eyes positioned ^ ^ ^0.1 run tp @e[sort=nearest,limit=1,tag=gm.vehicle] ~ ~-0.05 ~
