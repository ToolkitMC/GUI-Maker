...
$data modify storage barden:gui.all GUI[{GUI_ID:$(GUI_ID),username:$(username)}].PAGES[{page_number:$(page_number)}].layout[].components."minecraft:custom_data".BUTTON_INFO."sound" set value "empty"
$data modify storage barden:gui.all GUI[{GUI_ID:$(GUI_ID),username:$(username)}].PAGES[{page_number:$(page_number)}].layout[].components."minecraft:custom_data".BUTTON_INFO."item_modifier" set value ""
